...
*/
		...
		*/
		...
		public static function json_pretty($array, $pretty=true){
			if($pretty)
		...
		public static function json_array($string){
			$json_object = json_decode($string);
			$array = self::json_toarray($json_object);
			return $array;
		}
		
		
		//CONVERT JSON OBJECT completley to an array;
		public static function json_toarray($JSON){
			if(in_array(gettype($JSON), array('array', 'object'))){
				$JSON = (array)$JSON;
				foreach($JSON as $key => $value){
					$JSON[$key] = self::json_toarray($value);
				}
			} 
			return $JSON;
		}
		...
		public static function DumpTab($table){
			$str = '';
